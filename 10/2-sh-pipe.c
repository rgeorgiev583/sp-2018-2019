...
int main(int argc, const char* const* argv)
{
    while (1)
    {
        ...
        int op_count = 0, op_types[MAX_ARG_COUNT], subcommand_argv_positions[MAX_ARG_COUNT][2];
        op_types[op_count] = 0;
        ...
        for (int i = 0; i < arg_count; i++)
        {
            if (0 == strcmp(command_argv[i], "&&"))
            {
                op_count++;
                op_types[op_count] = 1;
                subcommand_argv_positions[op_count][0] = i + 1;
                subcommand_argv_positions[op_count][1] = -1;
                command_argv[i] = NULL;
            }
            ...
            {
                subcommand_argv_positions[op_count][1] = i + 1;
                command_argv[i] = NULL;
            }
        }
        op_count++;
